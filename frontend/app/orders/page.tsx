...
export default function OrderList() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-24 pb-12 px-5 max-w-lg mx-auto">
        <div className="mb-8">
          <h2 className="font-display text-5xl text-on-surface mb-2 uppercase tracking-tight">Đơn hàng</h2>
        ...
        <div className="flex gap-3 overflow-x-auto pb-6 no-scrollbar">
          {['TẤT CẢ', 'CHỜ XỬ LÝ', 'ĐANG GIAO', 'HOÀN THÀNH'].map((f, i) => (
            <button 
              key={f}
              className={`whitespace-nowrap px-6 py-2.5 rounded-full font-tech text-[10px] font-bold tracking-widest border transition-all ${i === 0 ? 'border-secondary bg-secondary text-white shadow-lg shadow-secondary/20' : 'border-outline-variant text-on-surface-variant hover:border-secondary'}`}
            >
              {f}
            </button>
          ))}
        ...
        <div className="space-y-6 mt-4">
          {orders.map((o) => (
            <motion.div 
              ...
              className="glass-card rounded-3xl p-5 flex flex-col gap-5 relative overflow-hidden group cursor-pointer border border-outline-variant/20"
            >
              ...
              <div className="flex gap-4 items-center">
                <div className="w-20 h-24 rounded-2xl overflow-hidden flex-shrink-0 bg-surface-container-high border border-outline-variant/10">
                  <img src={o.img} className="w-full h-full object-cover" alt={o.name} />
                </div>
                <div className="flex-grow">
                  <h3 className="font-tech text-md font-bold text-on-surface uppercase line-clamp-1">{o.name}</h3>
