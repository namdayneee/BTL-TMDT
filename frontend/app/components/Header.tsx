    ...
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-zinc-200 shadow-sm h-16">
        <div className="h-full max-w-7xl mx-auto px-5 md:px-8 lg:px-12 flex items-center relative">
          ...
          {/* MOBILE LEFT */}
          <div className="flex md:hidden items-center gap-3 shrink-0 z-50">
            {showBack ? (
              <button
                onClick={() => router.back()}
            ...
            {title && (
              <h1 className="font-tech text-sm font-semibold tracking-tight uppercase text-zinc-900 truncate max-w-45">
                {title}
              </h1>
            )}
          </div>
