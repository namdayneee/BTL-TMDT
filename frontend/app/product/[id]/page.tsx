  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-16 lg:flex lg:min-h-[calc(100vh-4rem)]">
        ...
        <section className="relative w-full aspect-3/4 lg:aspect-auto lg:w-5/12 xl:w-[45%] lg:sticky lg:top-16 lg:self-start lg:h-[calc(100vh-4rem)] overflow-hidden bg-surface-container-low shrink-0">
        ...
        {/* ── Info Panel ── */}
        <div className="flex-1 pb-32 lg:pb-12 lg:overflow-y-auto">
          <div className="px-5 md:px-8 lg:px-10 xl:px-14 max-w-2xl lg:max-w-none">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {['S', 'M', 'L', 'XL', 'XXL'].map(s => (
                      <button
                        key={s}
                        onClick={() => setSize(s)}
                        className={`min-w-15 h-12 border ${size === s ? 'border-2 border-secondary bg-secondary/5 text-secondary font-bold' : 'border-outline-variant'} rounded-xl font-tech text-xs transition-all`}
                      >
                        {s}
                      </button>
                    ))}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 p-5 bg-background/80 backdrop-blur-xl z-50">
        {addToCartButton}
      </div>
    </div>
  );
